...
import meristuff.blog.simplesamples.utils.CollectionsUtils;
import meristuff.blog.simplesamples.utils.PubliclyCloneable;
...
public class KeyListStorage<M, T> implements Cloneable {
  ...
  public void add(ListPlaceholder<M, T> placeholder, M key, List<T> things) {
    placeholder.level.add(key, things);
  }
  ...
  public void replacePlaceholder(ListPlaceholder<M, T> placeholder, KeyListStorage<M, T> otherStorage) {
    ...
    //replace in data
    CollectionsUtils.replace(levels, placeholder.level, otherStorage.levels);
    CollectionsUtils.replace(placeholders, placeholder, otherStorage.placeholders);
  }
  ...
  public KeyListStorage<M, T> clone() {
    try {
      ...
      clone.levels = CollectionsUtils.deeplyClonedLinkedList(levels);
  ...
  private static class Level<M, T> implements PubliclyCloneable {
    ...
    public Level<M, T> clone() {
      try {
        ...
        clone.storage = CollectionsUtils.deeplyClonedMapOfList(storage);
        return clone;
      } catch (CloneNotSupportedException e) {
        throw new IllegalStateException("Impossible state.");
      }
